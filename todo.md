...
## Motoko V2 design questions for chihiro

Triaged as deferred (V2) by ripley on 2026-05-14 after Motoko's retrospective V1 audit. These are design questions for chihiro at the next face-design cycle, not code hygiene items.

- [ ] **Accent palette role vocabulary (V2 chihiro decision).** Accent serves three concurrent roles across V1: *typography* (colons), *data signal* (Mechanical arrow glyphs, minute-arc strokes, Departures imminent-row tint), *structural chrome* (Departures bezel `--bezel-accent`, Horizon hairline cursor). No CONFIG vocabulary names which is which. Motoko independently confirmed the pattern across Calm, Mechanical, and Departures. The existing V2 housekeeping item "Per-face accent surfaces -- consider an `accentTargets` declaration in CONFIG" is the right fix. Promote it from "low priority" to Motoko-confirmed pattern across 3 faces. Decision needed at next face-design cycle: do `--bezel-accent` and the face-private accent reads consolidate into a single named accent system (`accentTargets: { typography: [...], dataSignal: [...], structuralChrome: [...] }`), or stay deliberately fragmented?

- [ ] **Calm slot cascade vs voice (V2 chihiro decision).** Calm's Auto Memory Doll voice says "nothing demands you," but the single rotator slot churns every 28-36 s with full KineticType per-character split-flap cascade -- the loudest motion on the face. Motoko: "This is the one motion on Calm that reads as a notification rather than a slow exhale." Mechanical removed the rotator entirely; Horizon disabled its macro; Editorial uses 1200 ms cross-fade. Calm is the last consumer of the V0 rotator vocabulary at full intensity. Decision needed: keep split-flap on Calm (V0 heritage, signature interaction) or soften to fade? Touches KineticType which is shared with Departures -- refactoring requires care.

- [ ] **Departures 6h column-width permutation is instant-swap (V2 chihiro decision, low priority).** Departures voice says "Solari is its small board," implying persistence -- but the 6h column-rotation logic performs an instant DOM-level column-width swap with no transition. Voice and macro behavior contradict mildly. Decision: animate the column permutation (per-column flap cascade) or accept the instant swap as a 6h-cadence non-event? User has not flagged this; user-perceptible only if they observe it live.
...
- [ ] **Per-face accent surfaces** -- Calm uses colon; Mechanical adds minute-arc; Departures adds imminent-row tint and flap-pair headline; Editorial adds colon only; Horizon uses gold hairline and colon (accentSkyTrack=false is a reserved V2 hook; gold is fixed in V1 because no face's accent is modulated by sky altitude). Consider a per-face `accentTargets` declaration in CONFIG to make surfaces explicit. V2 housekeeping candidate. See "Motoko V2 design questions for chihiro" above -- this item is Motoko-confirmed across 3 faces and should be addressed at the next face-design cycle.
