...
- `accentSkyTrack` face property -- a face may declare `accentSkyTrack: false` at the top level of its face object; `SkyColorModule.update()` reads this flag via a conditional that is a reserved/no-op branch in V1; the sky-altitude `--type-accent` modulation it would skip does not exist in V1 (removed in a pre-V1 cleanup), so the flag has no runtime effect today; when V2 re-introduces accent modulation, Horizon's opt-out activates without a code change; Horizon is the only V1 face to declare the opt-out intent; modulation of `--type-primary`, `--type-secondary`, and `--type-tertiary` is unaffected
...
- [x] Phase 20: Horizon face -- HorizonFace with full-stage 1180x820 SVG diagram, sun and moon arcs, 25 hour ticks, phase-correct crescent, 1 px gold hairline cursor, 220 px Manrope 200 big-time, accentSkyTrack opt-out (reserved V2 hook), macro shift disabled for Horizon in Phase 20.1; cross-fade transition vocabulary removed in 2026-05-14 post-V1 housekeeping
